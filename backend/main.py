...
async def attack_impact(attack_id: str, request: Request):
    ...
    if target.get("kind") == "mining" and target.get("status") == "Occupied":
        ...
        for op_id, op_data in GAME_STATE.get("active_attacks", {}).items():
            if op_data.get("target_id") == target_id and op_data.get("phase") == "occupying":
                defender_op_id = op_id  # <--- KITA SIMPAN ID PASUKAN AKUN A DI SINI
    ...
    if target.get("kind") == "mining":
        if success:
            ...
            if defender_op_id:
                battle_log.append("- Seranganmu dipukul mundur oleh komandan yang menjaga lahan ini!")
                
                # Beri tahu Pemilik Lama (Akun A) bahwa dia menang bertahan
                if defender_op_id in GAME_STATE.get("active_attacks", {}):
                    def_op = GAME_STATE["active_attacks"][defender_op_id]
                    if not isinstance(def_op.get("battle_log"), list):
                        def_op["battle_log"] = []
                    def_op["battle_log"].extend([
                        "",
                        "🛡️ BASE DEFENDED SUCCESSFULLY!",
                        f"- Seseorang mencoba merebut lahanmu, namun pertahananmu terlalu kuat!",
                        "- Melanjutkan operasi penambangan."
                    ])
                    GAME_STATE["active_attacks"][defender_op_id] = def_op
            else:
...
async def attack_recall(attack_id: str, request: Request):
    ...
    phase = active_attack.get("phase")
    
    # === PERBAIKAN: IZINKAN RECALL SAAT OUTBOUND MAUPUN OCCUPYING ===
    if phase not in ["occupying", "outbound"]:
        raise HTTPException(status_code=400, detail="Pasukan tidak bisa ditarik mundur pada fase ini.")

    now = time.time()
    target_id = active_attack.get("target_id")

    # 1. JIKA PASUKAN SEDANG DI JALAN (OUTBOUND)
    if phase == "outbound":
        started_at = float(active_attack.get("started_at", now))
        elapsed = max(1, int(now - started_at)) # Berapa lama mereka sudah jalan
        outbound_total = int(active_attack.get("outbound_seconds", 30))
        
        # Waktu pulang sama dengan waktu yang sudah ditempuh
        return_time = min(elapsed, outbound_total)

        active_attack["phase"] = "returning"
        active_attack["return_at"] = now + return_time
        active_attack["impact_at"] = None # Batalkan impact
        
        active_attack["battle_log"].append("")
        active_attack["battle_log"].append("⚠️ MISSION ABORTED!")
        active_attack["battle_log"].append("- Komandan membatalkan serangan di tengah jalan.")
        active_attack["battle_log"].append(f"- Pasukan berputar balik. Tiba di markas dalam {return_time} detik.")

    # 2. JIKA PASUKAN SEDANG MENAMBANG (OCCUPYING)
    elif phase == "occupying":
